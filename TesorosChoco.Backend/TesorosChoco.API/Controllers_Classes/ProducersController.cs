...
public class ProducersController : ControllerBase
{
    ...
    /// <summary>
    /// Obtiene productores destacados
    /// </summary>
    /// <returns>Lista de productores destacados</returns>
    [HttpGet("featured")]
    [ProducesResponseType(typeof(IEnumerable<ProducerDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<IEnumerable<ProducerDto>>> GetFeaturedProducers()
    {
        try
        {
            _logger.LogInformation("Getting featured producers");
            
            var producers = await _producerService.GetFeaturedProducersAsync();
            
            _logger.LogInformation("Retrieved {Count} featured producers", producers.Count());
            return Ok(producers);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error getting featured producers");
            return StatusCode(500, new { error = "Internal server error", message = "An error occurred while getting featured producers" });
        }
    }
}
